# SiennaTemplate.jl

```@meta
CurrentModule = SiennaTemplate
```

## Overview

`SiennaTemplate.jl` is a [`Julia`](http://www.julialang.org) package that provides blah blah

## About

`SiennaTemplate` is part of the NLR National Laboratory of the Rockies's
[Sienna ecosystem](https://www.nrel.gov/analysis/sienna.html), an open source framework for
scheduling problems and dynamic simulations for power systems. The Sienna ecosystem can be
[found on github](https://github.com/NLR-Sienna/Sienna). It contains three applications:

  - [Sienna\Data](https://github.com/NLR-Sienna/Sienna?tab=readme-ov-file#siennadata) enables
    efficient data input, analysis, and transformation
  - [Sienna\Ops](https://github.com/NLR-Sienna/Sienna?tab=readme-ov-file#siennaops) enables
    enables system scheduling simulations by formulating and solving optimization problems
  - [Sienna\Dyn](https://github.com/NLR-Sienna/Sienna?tab=readme-ov-file#siennadyn) enables
    system transient analysis including small signal stability and full system dynamic
    simulations

Each application uses multiple packages in the [`Julia`](http://www.julialang.org)
programming language.
